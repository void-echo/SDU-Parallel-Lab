...
static inline double calcOneChebyshevDistance(int dot_id1, int dot_id2,
    ...
    for (int i = 0; i < m; i++) {
        // THIS COST MOST TIME!!!
        double diff = fabs(euclidean_distance[small_cache[th][i] + dot_id1] -
                           euclidean_distance[small_cache[th][i] + dot_id2]);
        // double diff = 0;
        if (diff > max) {
            max = diff;
        }
...
int main(int argc, char *argv[]) {
    ...
    if (!DEBUG) {
        ...
        small_cache = (int**)malloc(sizeof(int*) * thread_count);
        for (int i = 0; i < thread_count; i++) {
            small_cache[i] = malloc(sizeof(int) * m);
            for (int j = 0; j < m; j++) {
                small_cache[i][j] = 0;
            }
        }
