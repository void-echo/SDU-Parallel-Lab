...
// // DO NOT USE THIS FUNCTION. IT IS MEMORY-INEFFICIENT.
...
static int *comb, *tmp;
static combination *next_combination() {
...
static inline void sniff_object_array() {
    ...
    // it does not print all combinations, but only print the first 10 and the last 10 pivot combinations.
    // It is useful when n is large, and m is small.
...
int main(int argc, char *argv[]) {
    ...
    if (!DEBUG) {
        ...
        // object = (struct combination **)malloc(sizeof(struct combination *) *
        //                                        pow(n, nums_p));
        ...
        printf("DEBUG MODE\n");
        int malloc_times = 1000;
        int __n__ = 500;
        struct timeval start, end;
        gettimeofday(&start, NULL);
        float **my_matrix = (float **)malloc(sizeof(float *) * __n__);
        for (int i = 0; i < malloc_times; i++) {
            for (int j = 0; j < __n__; j++) {
                my_matrix[j] = (float *)malloc(sizeof(float) * __n__);
            }
        }
        for (int j = 0; j < __n__; j++) {
            free(my_matrix[j]);
