...
static short unsigned int thread_count = 6;
static double *points;
static short unsigned int n, dim, nums_p, p_dot_id;
...
static short unsigned int m = 2;
static double *euclidean_distance;

typedef struct {
    short unsigned int *values;
    double cost;
} combination;
...
static INLINE double calcOneChebyshevDistance(short unsigned int dot_id1, short unsigned int dot_id2,
                                              short unsigned int tid) {
    double max = 0;
    short unsigned int d1 = tid * m * n;
    for (short unsigned int i = 0; i < m; i++) {
        short unsigned int d_1 = d1 + i * n + dot_id1;
        short unsigned int d_2 = d1 + i * n + dot_id2;
...
static void INLINE calc_all_combinations_and_store_in_object() {
    ...
    short unsigned int *combi_ = (short unsigned int *)malloc(sizeof(short unsigned int) * m);
    for (short unsigned int i = 0; i < m; i++) {
        combi_[i] = i;
    }
    int count_ = 0;
...
static void INLINE dmp_object_array_to_file() {
    ...
    for (int i = len - 1; i >= len - 1000; i--) {
        for (int j = 0; j < m; j++) {
            fprintf(fp, "%d", (int)object[i].values[j]);
            if (j != m - 1) {
                fprintf(fp, " ");
            }
        }
...
int main(int argc, char *argv[]) {
    ...
    if (!DEBUG) {
        int _dim, _n, _nums_p, _thread_count;
        _thread_count = 6;
        ...
        while ((opt = getopt(argc, argv, "f:t:")) != -1) {
            switch (opt) {
                ...
                case 't':
                    ...
                    thread_count = (unsigned short int) _thread_count;
        ...
        fscanf(file, "%d", &_dim);
        fscanf(file, "%d", &_n);
        fscanf(file, "%d", &_nums_p);
        dim = (short unsigned int) _dim;
        n = (short unsigned int) _n;
        nums_p = (short unsigned int) _nums_p;
        m = nums_p;
        printf("dim = %d, point number = %d, pivot numver = %d, thread number = %d, using file %s\n", _dim, _n,
               _nums_p, _thread_count, filename);
        ...
        for (int i = 0; i < c_n_m(n, m); i++) {
            object[i].values = (short unsigned int *)malloc(sizeof(short unsigned int) * m);
            object[i].cost = 0;
        }
