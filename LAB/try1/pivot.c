...
double SumDistance(const int k, const int n, const int dim, double *coord,
    ...
    for (i = 0; i < n; i++) {
        ...
        for (ki = 0; ki < k; ki++) {
            ...
            for (j = 0; j < dim; j++) {
                distance +=
                    pow(coord[pivoti * dim + j] - coord[i * dim + j], 2);
            }
    ...
    for (i = 0; i < n; i++) {
        ...
        for (j = 0; j < n; j++) {
            ...
            for (ki = 0; ki < k; ki++) {
                double dis =
                    fabs(rebuiltCoord[i * k + ki] - rebuiltCoord[j * k + ki]);
...
// Recursive function Combination() : combine pivots and calculate the sum of
// distance while combining different pivots. ki  : current depth of the
// recursion k   : number of pivots n   : number of points dim : dimension of
// metric space M   : number of combinations to store coord  : coordinates of
...
void Combination(int ki, const int k, const int n, const int dim, const int M,
            ...
            for (a = M; a > 0; a--) {
                if (maxDistanceSum[a] > maxDistanceSum[a - 1]) {
                    ...
                    for (kj = 0; kj < k; kj++) {
                        ...
                        maxDisSumPivots[a * k + kj] =
                            maxDisSumPivots[(a - 1) * k + kj];
            ...
            for (a = M; a > 0; a--) {
                if (minDistanceSum[a] < minDistanceSum[a - 1]) {
                    ...
                    for (kj = 0; kj < k; kj++) {
                        ...
                        minDisSumPivots[a * k + kj] =
                            minDisSumPivots[(a - 1) * k + kj];
    ...
    for (i = pivots[ki - 1] + 1; i < n; i++) {
        pivots[ki] = i;
        Combination(ki + 1, k, n, dim, M, coord, pivots, maxDistanceSum,
...
int main(int argc, char *argv[]) {
    ...
    for (i = 0; i < n; i++) {
        ...
        for (j = 0; j < dim; j++) {
            fscanf(file, "%lf",
                   &coord[i * dim +
                          j]);  /// TODO HERE: invoke fscanf() too many times
        }
    }
    fclose(file);
    ...
    for (i = 0; i < M; i++) {
        minDistanceSum[i] = 1.79769313486231570814527423731704357e+308L;
    }
    ...
    Combination(0, k, n, dim, M, coord, &temp[1], maxDistanceSum,
                maxDisSumPivots, minDistanceSum, minDisSumPivots);
