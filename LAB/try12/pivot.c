...
static inline double calcOneChebyshevDistance(int dot_id1, int dot_id2,
    ...
    double max = 0;
    for (int i = 0; i < m; i++) {
        int pivot_id = small_cache[th * m + i];
        // THIS COST MOST TIME!!!
        double diff = fabs(get_euclidean_distance(pivot_id, dot_id1) -
                           get_euclidean_distance(pivot_id, dot_id2));
        // double diff = 0;
        if (diff > max) {
            max = diff;
        }
...
int main(int argc, char *argv[]) {
    ...
    if (!DEBUG) {
        ...
        small_cache = (int *)malloc(sizeof(int) * thread_count * m);
        // set small cache to 0
        for (int i = 0; i < thread_count * m; i++) {
            small_cache[i] = 0;
        }
