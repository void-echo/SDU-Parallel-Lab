...
static INLINE double get_euclidean_distance(int id1, int id2) {
    return euclidean_distance[id1][id2];
}
...
static INLINE double calcOneChebyshevDistance(int dot_id1, int dot_id2,
    ...
    double max = 0;
    for (int i = 0; i < m; i++) {
        double diff = fabs(cache_eu_dist[tid][i][dot_id1] -
                           cache_eu_dist[tid][i][dot_id2]);
        // double diff = 0;
        if (diff > max) {
            max = diff;
        }
...
int main(int argc, char *argv[]) {
    ...
    if (!DEBUG) {
        ...
        for (int i = 0; i < thread_count; i++) {
            cache_eu_dist[i] = (double **)malloc(sizeof(double *) * m);
            for (int j = 0; j < m; j++) {
                cache_eu_dist[i][j] = (double *)malloc(sizeof(double) * n);
            }
        }
        ...
        fclose(file);
        gettimeofday(&start, NULL);
        euclidean_distance = (double **)malloc(sizeof(double *) * n);
        for (int i = 0; i < n; i++) {
            euclidean_distance[i] = (double *)malloc(sizeof(double) * n);
        }
